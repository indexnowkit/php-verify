...
относительный `<link rel="canonical">` разрешается против `<base href>` документа, если он объявлен, — так же, как это
делает краулер.

Pre-flight ходит **своим** PSR-18-клиентом, который находит ядро, с `max_redirects: 0`, — а не клиентом приложения из
`http.client`, даже если он задан. Pre-flight обязан видеть ответы 3xx сам (`verify.redirect`, `verify.max_redirects`,
проверка хоста цели редиректа), а PSR-18 не позволяет запретить следование редиректам чужому клиенту. `http.client`
по-прежнему несёт POST-отправки; `check` печатает это разделение (`verify.transport`).
